...
class ProtExclViewFilter(EMProtocol, ProtTomoBase):
    ...
    def _defineParams(self, form):
        ...
        lineShift = form.addLine('Filter by shift (px)',
                                help='This is the minimum/maximum shift allowed along the X or Y direction.'
                                     ...
                                     'is allowed.')
        lineShift.addParam('minShiftX', FloatParam, default=0.1, label="Max shift in X")
        ...
        lineTilt = form.addLine('Filter by tilt angle (deg)',
                                help='This is the minimum/maximum tilt angle allowed.'
                                     'Only the tilt images with tilt angles in the range min<tilt<max '
                                     'will be preserved in the output tilt series.')
        ...
        form.addParam('minViews',
                      ...
                      help='Minimum number of views to be included in the output tilt series.')
    ...
    def excludeViewFilteringStep(self):
        ...
        xdim, ydim, _ = inputData.getFirstEnabledItem().getDim()
        ...
        for ts in inputData:
            ...
            maxTiltAcq = tomoAcq.getAngleMax()
            minTiltAcq = tomoAcq.getAngleMin()
            newMaxTilt = maxTiltAcq if maxTiltAcq>maxTiltThreshold else maxTiltThreshold
            newMinTilt = minTiltAcq if minTiltAcq<minTiltThreshold else minTiltThreshold
            tomoAcq.setAngleMax(newMaxTilt)
            tomoAcq.setAngleMax(newMinTilt)
    ...
    def _getInTsSet(self, returnPointer: bool = False) -> Union[SetOfTiltSeries, Pointer]:
        inTsPointer = getattr(self, 'inTsSet')
